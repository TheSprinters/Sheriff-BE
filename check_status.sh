...
if lsof -ti:8325 > /dev/null 2>&1; then
    echo "✅ Backend is RUNNING"
    echo "   Process IDs: $(lsof -ti:8325 | tr '\n' ' ')"
else
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8325/api/post/all)

if [ "$HTTP_CODE" == "200" ]; then
    echo "✅ API is responding (HTTP $HTTP_CODE)"
    echo ""
    echo "📊 Sample response:"
    curl -s http://localhost:8325/api/post/all | head -100
...
echo "🌐 Access Points:"
echo "========================================"
echo "   API: http://localhost:8325/api/post/all"
